...
function recordToArray(rec: SocialLinksRecord | undefined): SocialLink[] {
  ...
  return Object.entries(rec)
    ...
    .filter(([k]) => SUPPORTED.includes(k.toLowerCase() as SocialLink["platform"]))
    .map(([platform, url]) => ({
  ...
  useEffect(() => {
    ...
    const onMessage = (e: MessageEvent) => {
      const data = e.data as
        | { type?: string; section?: string; patch?: SocialLinksRecord | SocialLink[] }
        | null;
      if (
        !data ||
        typeof data !== "object" ||
