...
export function PreviewLinksClient({
  ...
  useEffect(() => {
    ...
    const onMessage = (e: MessageEvent) => {
      const data = e.data as
        | { type?: string; section?: string; patch?: LinkCategory[] }
        | null;
      if (
        !data ||
        typeof data !== "object" ||
  ...
  return (
    <section className="mx-auto max-w-4xl px-4 py-8">
      <h1 className="mb-2 text-3xl font-bold tracking-tight">Enlaces de Interés</h1>
      ...
      {categories.length === 0 ? (
        ...
        <div className="space-y-10">
          {categories.map((cat, i) => (
            <div key={i}>
              <h2 className="mb-4 text-xl font-bold">{cat.name || "Sin nombre"}</h2>
